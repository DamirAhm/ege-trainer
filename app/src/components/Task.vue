<template>
	<div class="problem" :style="{ borderColor }">
		...
		<div class="problem-text" v-if="text !== undefined && text.length > 0">
			...
			<button class="btn" v-else @click="textVisible = true">
				Открыть текст
			</button>
		...
		<button v-else class="problem-check btn" @click="handleCheck">
			Проверить
		</button>
		<div
...
export default {
	...
	methods: {
		handleCheck() {
			...
			if (
				this.answer.includes(this.userInput) &&
				this.answerType !== answerTypes.html
			) {
